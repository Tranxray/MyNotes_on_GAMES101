                                ...
                                const std::array<int, 3>& dirIsNeg) const
{
    ...
    if (dirIsNeg[0] == 0)std::swap(t_x_min, t_x_max);
    if (dirIsNeg[1] == 0)std::swap(t_y_min, t_y_max);
    if (dirIsNeg[2] == 0)std::swap(t_z_min, t_z_max);

    float t_enter = std::max(t_x_min, std::max(t_y_min, t_z_min));
    float t_exit = std::min(t_x_max, std::max(t_y_max, t_z_max));
